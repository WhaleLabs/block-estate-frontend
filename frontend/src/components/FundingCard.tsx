...
export function FundingCard({ id, pictures, title, location, price, raised, holders, status, totalTokens, loading }:
    { id: number, pictures: string[], title: string, location: string, price: number, raised: number, holders: object, status: string, totalTokens: number, loading: boolean }) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => setProgress((raised / price * 100)), 500)
                ...
                <div className='relative'>
                            ...
                            {pictures.map((image, index) => (
                                <>
                                    <img
                                        ...
                                        src={image}
                                        alt={`Slide ${index + 1}`}
                                        className={`absolute aspect-square rounded-xl object-cover hover:brightness-9 top-0 left-0 w-full ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'
                                    ...
                                    <img
                                        src={image}
                                        alt={`Slide ${index + 1}`}
                                        className={`-z-10 absolute aspect-square rounded-xl object-cover hover:brightness-9 top-0 left-0 w-full ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'
                                            }`}
                                    />
                                </>
                            ))}
                    ...
                    <div className="flex flex-row">
                        ...
                        <div className="py-2 w-[40%] flex flex-col justify-center items-end px-2">
                            <p className="text-md font-semibold text-primary-text">${raised/1000}/${price/1000} k</p>
                            <p className="text-sm text-primary-600">{progress.toFixed(2)}%</p>
                        </div>
                    </div>
                </div>
            }
